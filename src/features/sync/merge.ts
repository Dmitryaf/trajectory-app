...
export function mergeCloudSnapshots(baseInput: unknown, localInput: unknown, remoteInput: unknown): ExportPayload {
  ...
  return normalizeSnapshot({
    ...
    dailyEntries: mergeKeyed(base.dailyEntries, local.dailyEntries, remote.dailyEntries, (entry) => entry.date),
    results: mergeJournalRecords(base.results, local.results, remote.results),
    lifeEvents: mergeJournalRecords(base.lifeEvents ?? [], local.lifeEvents ?? [], remote.lifeEvents ?? []),
    weeklyReviews: mergeKeyed(base.weeklyReviews, local.weeklyReviews, remote.weeklyReviews, (review) => review.weekStart),
...
function mergeKeyed<T extends object>(base: T[], local: T[], remote: T[], keyOf: (item: T) => string): T[] {
...
function mergeJournalRecords<T extends ResultRecord | LifeEventRecord>(base: T[], local: T[], remote: T[]): T[] {
  ...
  return mergeKeyed(base, remappedLocal, remote, keyOf);
}
...
function mergeObjectFields<T extends KeyedRecord>(base: T, local: T, remote: T): T {
