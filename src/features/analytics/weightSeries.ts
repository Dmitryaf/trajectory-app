...
export function buildWeightSeries(
  ...
  return dates.filter((date) => date <= dataThrough).map((date) => {
    ...
    const rolling = windowValues.length >= 2
      ? Math.round((windowValues.reduce((sum, value) => sum + value, 0) / windowValues.length) * 10) / 10
      : null;
